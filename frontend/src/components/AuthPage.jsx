...
export default function AuthPage({ setUser, setRole, setPage, setToast, pageOptions }) {
  const ownerRegister = !!pageOptions?.ownerRegister;
  const [mode, setMode] = useState(() => ownerRegister ? "register" : "login");
  const [login, setLogin] = useState(initialLogin);
  const [register, setRegister] = useState(() =>
    ownerRegister ? { ...initialRegister, hostParking: true } : initialRegister
  );
  ...
  const handleRegister = async (e) => {
    ...
    if (!firstName) { setError("Podaj imię."); return; }
    if (!lastName) { setError("Podaj nazwisko."); return; }
    if (!email) { setError("Podaj adres e-mail."); return; }
    if (!EMAIL_RE.test(email)) { setError("Podaj poprawny adres e-mail, np. jan@example.com."); return; }
    if (!register.password) { setError("Podaj hasło."); return; }
    if (register.password.length < 6) { setError("Hasło musi mieć co najmniej 6 znaków."); return; }
    if (plate && (plate.length < 2 || plate.length > 10)) {
      setError("Tablica rejestracyjna musi mieć od 2 do 10 znaków.");
      return;
    }
    if (register.password !== register.confirmPassword) { setError("Hasła muszą być takie same."); return; }
    if (!register.terms) { setError("Zaakceptuj regulamin, aby utworzyć konto."); return; }
    ...
    <div className="auth-page fin">
      <div className="auth-copy">
        <div className="land-badge">
          <I.Shield /> {ownerRegister ? "Rejestracja właściciela parkingu" : "Konto kierowcy"}
        </div>
        <h1>
          {ownerRegister
            ? "Zarejestruj swój parking"
            : mode === "login"
              ? "Wróć do swoich rezerwacji"
              : "Utwórz konto w parkuj.my"}
        </h1>
        <p>
          {ownerRegister
            ? "Utwórz konto, a następnie dodasz parking w kreatorze. Kierowcy będą mogli rezerwować miejsca online."
            : "Zarządzaj pojazdami, rezerwacjami i płatnościami z jednego miejsca. Numer rejestracyjny może działać jak bilet przy wjeździe."}
        </p>
      </div>
